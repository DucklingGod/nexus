    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="rounded-xl border border-nexus-border bg-nexus-surface/40 p-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <input value={sshForm.key_path} onChange={e => setSshForm({ ...sshForm, key_path: e.target.value })} placeholder="Private key path (optional, e.g. ~/.ssh/id_ed25519)"
            className="col-span-2 rounded-xl border border-nexus-border bg-nexus-surface px-3 py-2 text-sm text-nexus-fg placeholder-nexus-muted outline-none focus:border-nexus-accent" />
        </div>
        <div className="mt-3 flex items-center gap-2">
          <button onClick={saveSshHost} className="rounded-xl bg-nexus-accent px-4 py-2 text-sm font-medium text-black hover:opacity-90">{sshEditingId ? "Update" : "Add host"}</button>
          {sshEditingId && <button onClick={() => { setSshEditingId(null); setSshForm({ name: "", host: "", user: "", port: "22", key_path: "" }); }} className="rounded-xl border border-nexus-border px-4 py-2 text-sm text-nexus-muted hover:bg-nexus-surface">Cancel</button>}
        </div>
      </div>
      ...
      {sshHosts.length === 0 ? (
        ...
        <div className="flex flex-col gap-2">
          {sshHosts.map(h => (
            <div key={h.id} className="rounded-xl border border-nexus-border bg-nexus-surface/40 p-3">
