    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex flex-col gap-2">
        ...
        <button onClick={handleAddDoc} disabled={saving || !docContent.trim()}
          className="self-start rounded-xl bg-nexus-accent px-5 py-2 text-sm font-medium text-black hover:opacity-90 disabled:opacity-50">Add text</button>
      </div>
      <div className="flex gap-2">
        ...
        <button onClick={handleIngestFile} disabled={saving || !docPath.trim()}
          className="rounded-xl border border-nexus-border px-4 py-2 text-sm text-nexus-fg hover:bg-nexus-surface disabled:opacity-50">Ingest file</button>
      </div>
      ...
      <div className="flex flex-col gap-2 border-t border-nexus-border/40 pt-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button onClick={handleAddFolder} className="rounded-xl border border-nexus-border px-3 py-1.5 text-xs text-nexus-fg hover:bg-nexus-surface">+ Add folder</button>
            <button onClick={handleSyncFolders} disabled={folderSyncing || folders.length === 0} className="rounded-xl bg-nexus-accent px-3 py-1.5 text-xs font-medium text-black hover:opacity-90 disabled:opacity-50">{folderSyncing ? "Syncing…" : "Sync now"}</button>
          </div>
        </div>
        {folders.length === 0 ? (
          ...
          <div key={f} className="flex items-center justify-between rounded-xl border border-nexus-border bg-nexus-surface px-3 py-2">
      ...
      <div className="flex flex-col gap-2 border-t border-nexus-border/40 pt-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button onClick={handleAddVault} className="rounded-xl border border-nexus-border px-3 py-1.5 text-xs text-nexus-fg hover:bg-nexus-surface">+ Add vault</button>
            <button onClick={handleSyncVaults} disabled={vaultSyncing || vaults.length === 0} className="rounded-xl bg-nexus-accent px-3 py-1.5 text-xs font-medium text-black hover:opacity-90 disabled:opacity-50">{vaultSyncing ? "Syncing…" : "Sync now"}</button>
          </div>
        </div>
        ...
        {vaults.length === 0 ? (
          ...
          <div key={v} className="flex items-center justify-between rounded-xl border border-nexus-border bg-nexus-surface px-3 py-2">
      ...
      <div className="flex flex-col gap-2">
        ...
        {docs.length === 0 ? (
          ...
          <div key={d.id} className="flex items-center justify-between rounded-xl border border-nexus-border bg-nexus-surface px-3 py-2">
